...
class DSET_processor:
    ...
    def _create_example_oeecfc(self, lines):
        ...
        en2cn_dict = {'subject': '主体', 'object': '客体', 'tool': '工具', 'material': '材料', 'manner': '方式',
                      ...
                      'placeAdverb': '地点副词', 'moodAdverb': '情态/语气副词'}
        ...
        for line in lines:
            ...
            for event_idx, event in enumerate(events):
                ...
                for i in range(len(sent_dir)):
                    event_trigger['offset'] = offset
                    if i < len(sent_dir) - 1:
                        if event_trigger['end'] < first_word_locs[i + 1] and event_trigger['start'] >= first_word_locs[
                            i]:
    ...
    def _create_example_rams(self, lines):
        ...
        print("{} examples collected. {} arguments dropped.".format(len(examples), self.invalid_arg_num))#打印收集到的示例数量和因超出窗口而丢弃的论元数。
        return examples
    ...
    def create_example(self, file_path):
        ...
        elif self.args.dataset_type=='oeecfc':
            lines = self._read_jsonlines(file_path)
            return self._create_example_oeecfc(lines)
    ...
    def generate_dataloader(self, set_type):
        ...
        if self.args.dataset_type == 'OEE_CFC':
            features = self.convert_examples_to_features_oeecfc(examples)
        else:
