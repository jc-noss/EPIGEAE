...
def get_best_span(start_logit, end_logit, old_tok_to_new_tok_index, max_span_length):
    start_logit = start_logit.to('cpu').detach().numpy()
    end_logit = end_logit.to('cpu').detach().numpy()
    start_indices = np.array([x[0] for x in old_tok_to_new_tok_index], dtype=np.int32)
    end_indices = np.array([x[1] for x in old_tok_to_new_tok_index], dtype=np.int32)
    context_length = len(start_indices)
    valid_spans = np.full((context_length, max_span_length), -np.inf, dtype=np.float32)
    for start in range(context_length):
        max_end = min(start + max_span_length + 1, context_length)
        valid_length = max_end - start - 1
        if valid_length > 0:
            start_idx = start_indices[start]
            end_idx_slice = end_indices[start: max_end - 1]
            valid_spans[start, :valid_length] = (
                    start_logit[start_idx] +
                    end_logit[end_idx_slice]
            )
    initial_score = start_logit[0] + end_logit[0]
    valid_spans_flat = valid_spans.ravel()
    if valid_spans_flat.size == 0 or valid_spans_flat.max() <= initial_score:
        return (0, 0)
    max_flat_idx = np.argmax(valid_spans_flat)
    start = max_flat_idx // max_span_length
    span_length = max_flat_idx % max_span_length
    end = start + span_length + 1
    end = min(end, context_length)
    return (
        start_indices[start],
        end_indices[end - 1] if end > 0 else 0
    )
# def get_best_span(start_logit, end_logit, old_tok_to_new_tok_index, max_span_length):
#     # time consuming
